...
/* A
describe('graphmc dijkstra algorithm', () => {
  it('should correctly find shortest path between two vertices', () => {
    const graph = new graphmc.Graph(7);
    graph.place(0, 0, 0);
    graph.place(1, 1, 1);
    graph.place(2, 1, 3);
    graph.place(3, 2, 0);
    ...
    graph.connect(4, 6);
    graph.connect(5, 6);
    const result = graphmc.dijkstra(graph, 0, 6);
    assert(result < 5.657 && result > 5.656, 'distance should be 5.6568');
  });
});
